...
public class EnemyDamageHandler : MonoBehaviour
{
    ...
    public typeOfMonster monsterType;
    private PlayerStats stats;
    private float timeBeforeDestroy = 0.25f;

    public int arrowsHitMe;
    ...
    {
        stats.experienceGained += experienceWorth;

        foreach (AbilityInventorySlot abilitySlot in PlayerInventoryHandler.abilityInventory.Container.abilities)
        {
            if (abilitySlot.ability.monsterTypeToLearn.Equals(monsterType))
            {
                abilitySlot.ability.UpdateAbility(experienceWorth);
                Debug.Log("checked");
            }
    ...
    public IEnumerator DelayedDestroyThis()
    {
        ...
        for (int i = 0; i < arrowsHitMe; i++) {
            Instantiate(arrowObject, this.gameObject.transform.position, Quaternion.identity);
        }
    }
